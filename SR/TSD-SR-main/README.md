...
#### Step 5: Multi-scenario benchmark sweep (recommended for tuning)
You can run multiple inference parameter combinations and optionally stress-test input degradations in one command.
...
--lora_dir checkpoint/tsdsr-mse \
--embedding_dir dataset/default \
--dataset "DRealSR|imgs/StableSR_testsets/DrealSRVal_crop128/test_LR|imgs/StableSR_testsets/DrealSRVal_crop128/test_HR" \
--scenario_set quick
```

Extended preset with perturbation stress test:
```
python script/run_benchmark_scenarios.py \
--pretrained_model_name_or_path /path/to/your/sd3 \
--lora_dir checkpoint/tsdsr-mse \
...
- **Modify** the dataset path in `data/data.py` and ensure the directory structure:
```
your_training_datasets/ # Example: FLICKR2K/
    ...
    └── prompt_txt
        ...
        └── ...
```
- Run `data/process.py` with the base model you want to use for cache generation:
```
python data/process.py --pretrained_model_name_or_path /path/to/your/sd3_model
```
This step is designed to **reduce GPU memory overhead during training**. Since SD3’s tokenizer and text encoder require significant parameter storage, we pre-compute their outputs and load them directly during training. Similarly, we also pre-process the HR (High-Resolution) latent space tensors to optimize memory efficiency. If you change the teacher/student base model, regenerate these caches with the same `--pretrained_model_name_or_path` you plan to train against. The final data folder will be like this:
```
your_training_datasets/ # Example: FLICKR2K/
    └── gt
        └── 0000001.png # GT images, (3, 512, 512)
